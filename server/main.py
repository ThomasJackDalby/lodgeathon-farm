...
from farmers import Farmer, FarmerPublic, FarmerCreate, FarmerUpdate, MoveCommand, OtherCommand
...
def post_farmer(farmer_id: int, farmer_update: FarmerUpdate):
    for command in farmer_update.commands:
        if command.type == "move":
            command = MoveCommand.model_validate(command)
        elif command.type == "other":
            command = OtherCommand.model_validate(command)
