...
from farmers import Farmer, FarmerPublic, FarmerCreate
...
engine = create_engine(sqlite_url, echo=True, connect_args={"check_same_thread": False}, poolclass=StaticPool)
...
def on_startup():
    ...
    with Session(engine) as session:
        ...
        for farmer_name in ["Matt", "Tom"]:
            farmer = Farmer(name=farmer_name, token="test", x=0, y=0)
            session.add(farmer)
            session.commit()
...
def post_farmer(farmer: FarmerCreate):
    with Session(engine) as session:
        farmer.x = 0
        farmer.y = 0
...
@app.get("/farmers/{farmer_id}", response_model=list[FarmerPublic])
def get_farmers(farmer_id: int):
    with Session(engine) as session:
        return session.exec(select(Farmer).where(Farmer.id == farmer_id)).all()
